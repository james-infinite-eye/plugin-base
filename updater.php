...
if (!defined('IE_API_URL')) {
    define('IE_API_URL', 'https://sneaky:peaky@resources.infinite-eye.com');
}
...
class IEWP_Updater
{
    ...
    private function get_repository_info()
    {
        if (is_null($this->plugin_api_response)) { // Do we have a response?
            $request_uri = sprintf(IE_API_URL . '/api/v1/index.php?access_token=%s&action=status&plugin=%s', $this->authorize_token, $this->id);
